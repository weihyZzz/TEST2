...
if(nAzi == 1 || nEl == 1) % 水平面或竖直面
    ...
    if(nAzi == 1)
        ...
        azimuthTemp = [azimuthGrid(ind(1)),azimuthGrid(ind(numberSourcesFound+1))];          
        pfEstAngles = [sort(abs(azimuthTemp))' elevation.*ones(numberSourcesFound,1)];    
%        pfEstAngles = [azimuthGrid(ind(1:numberSourcesFound))' elevation.*ones(numberSourcesFound,1)];

    end
    
    % Display
    % displayResults=1;figHandle=1;
    if (displayResults)
        figure(figHandle);
        if(nAzi == 1)
            ...
            plot(abs(azimuthTemp),specGlobal(ind(1:numberSourcesFound)),'*k','MarkerSize',15,'linewidth',1.5);
            xlabel('\theta (degrees)');
            ylabel('Angular spectrum');
            hold off;
