function [Yhat_FTM,T_NFK,V_NKT,G_NM,Q_FMM,cost] = t_FastMNMF2_offline(X,XX,N,K,maxIt,drawConv,v,trial,G_NM,T_NFK,V_NKT,Q_FMM)
...
if sum(size(T_NFK) ~= [N,F,K]) || sum(size(V_NKT) ~= [N,K,T]) || sum(size(G_NM) ~= [N,M]) %|| sum(size(Z) ~= [K,N])
    error('The size of input initial variable is incorrect.\n');
end
[Q_FMM, G_NM, T_NFK, V_NKT] = normalize(Q_FMM, G_NM, T_NFK, V_NKT, M);
Yhat_FTM = local_Yhat( T_NFK, V_NKT, G_NM, F, T, M, N); % (6) of [2] initial model tensor F *M *M 
%% Iterative update
...
if ( drawConv == true )
   ...
   for it = 1:trial
        ...
        [ Yhat_FTM, T_NFK, V_NKT] = init_local_iterativeUpdate(X, XX, Yhat_FTM, T_NFK, V_NKT, G_NM, F, T, K, N, M ,Q_FMM, v);
...
fprintf('Iteration:    ');
    ...
    for it = 1:maxIt
        fprintf('\b\b\b\b%4d', it);
        [ Yhat_FTM, T_NFK, V_NKT, G_NM, Q_FMM ] = local_iterativeUpdate(X, XX, Yhat_FTM, T_NFK, V_NKT, G_NM, F, T, K, N, M ,Q_FMM, v);
        cost(it+1) = local_cost( X, Yhat_FTM, F, T, M ,Q_FMM);
    end
    figure;
...
[Tnume ,Tdeno] = local_Tfrac( alpha, QY_FTM, Yhat_FTM, V_NKT, G_NM, F, T, K, M ,N); % F x K         % % (30) of [2]分子更新
...
%%% Iterative update %%%
function [ Yhat_FTM, T_NFK, V_NKT, G_NM, Q_FMM] = local_iterativeUpdate(X, XX, Yhat_FTM, T_NFK, V_NKT, G_NM, F, T, K, N, M ,Q_FMM,v)
...
G_NM = G_NM.*max(sqrt(Gnume./Gdeno),eps)+ 1e-10; %  (35) of [2]
...
for f = 1:F
    GQY_NFT(:,f,:) = G_NM * squeeze(alphaQY_IJM(f,:,:)).';% 分子中的 alpha * beta (30) of [2] size(G)  % F * N * M 
end
for n=1:N
    for t=1:T
...
%%% Vfrac %%%
function [ Vnume, Vdeno ] = local_Vfrac( alpha, QY_FTM,Yhat_FTM, T_NFK, G_NM, F, T, K, M ,N) % QY_IJM =|q*x|^2 * y.^-2 即是（31）of [2]除了g的部分
...
for f = 1:F
    GQY_NFT(:,f,:) = G_NM * squeeze(alphaQY_FTM(f,:,:)).';% 分子中的 alpha * beta (33) of [2] size(G)  % F * N * M 
end
for n=1:N
    for t=1:T
    GYhat_NFTM(n,:,t,:) = G_NM(n,:) ./ squeeze(Yhat_FTM(:,t,:));    % (32) of [2]  
...
for f = 1:F
    Gnumetmp(f,:,:) = squeeze(TV_NFT(:,f,:)) * squeeze(alphaQY_FTM(f,:,:));% 分子中t*v*alpha in (35) of [2] 
end
