% function [Yhat_FTM,T_NFK,V_NKT,G_FNM,Q_FMM,cost] = FastMNMF_offline(X,XX,N,K,maxIt,drawConv,G_FNM,T_NFK,V_NKT,Q_FMM)
function [Yhat_FTM,T_NFK,V_NKT,G_FNM,Q_FMM,cost] = FastMNMF_offline(X,XX,N,K,maxIt,drawConv,option,G_FNM,T_NFK,V_NKT,Q_FMM)
...
%% gradualinit
    ...
    if gradualinit
        ...
        for it = 1:initIt
            fprintf('\b\b\b\b%4d', it);
            [ Yhat_FTM, T_NFK, V_NKT, G_FNM, Q_FMM ] = local_iterativeUpdate(X, XX, Yhat_FTM, T_NFK, V_NKT, G_FNM, F, T, K, N, M ,Q_FMM, spec_indices{1},option);
        end
...
%%% Iterative update %%%
function [ Yhat_FTM, T_NFK, V_NKT, G_FNM, Q_FMM] = local_iterativeUpdate(X, XX, Yhat_FTM, T_NFK, V_NKT, G_FNM, F, T, K, N, M ,Q_FMM,indice,option)

%% %%% Update T %%%%%% N *F *K
QX_power = local_QX_power( Q_FMM, X, F, T, M, indice);
QY_FTM = (QX_power./(Yhat_FTM+eps).^2);% |q*x|^2 * y.^-2 in (20) of [2]
[Tnume ,Tdeno] = local_Tfrac( QY_FTM, Yhat_FTM, V_NKT, G_FNM, F, T, K, M ,N, indice); % F x K         % % (20) of [2]更新
T_NFK = T_NFK.*max(sqrt(Tnume./Tdeno),eps); % (20) of [2] 
 
Yhat_FTM = local_Yhat( T_NFK, V_NKT, G_FNM, F, T, M ,N, indice); % under 1 line of (15) in [2]

%% %%% Update V %%%%%% N *K *T
QY_FTM = (QX_power./(Yhat_FTM+eps).^2);
[Vnume ,Vdeno] = local_Vfrac( QY_FTM, Yhat_FTM, T_NFK, G_FNM, F, T, K, M ,N, indice); % F x K         % % (21) of [2]更新
...
%% %%% Update G %%%%%
  %%标准迭代
QY_FTM = (QX_power./(Yhat_FTM+eps).^2);
[Gnume ,Gdeno] = local_Gfrac( QY_FTM, Yhat_FTM, T_NFK, V_NKT, F, T, M ,N, indice); % F x K         % % (22) of [2]更新
G_FNM = G_FNM.*max(sqrt(Gnume./Gdeno),eps) + 1e-10; % (22) of [2]
...
%% 
%%% Riccati solver %%%
% function [ G_FNM ] = local_RiccatiSolver(QY_FTM,d_Y_FTM, T_NFK, V_NKT, G_FNM, F, T, N, M)%% (18) of [2]等价于（18），都是solution of Riccati equation
function [ G_FNM ] = local_RiccatiSolver(QY_FTM,d_Y_FTM, T_NFK, V_NKT, G_FNM, F, T, N, M, indice)%% (18) of [2]等价于（18），都是solution of Riccati equation
X = zeros(F,T,M,M);Y = zeros(F,T,M,M);G_FNMM = zeros(F,N,M,M);
for f = indice%1:F
    for t=1:T
        X(f,t,:,:) = diag(squeeze(QY_FTM(f,t,:)));
        Y(f,t,:,:) = diag(squeeze(d_Y_FTM(f,t,:)));
    end
    for n = 1:N
        G_FNMM(f,n,:,:) = diag(squeeze(G_FNM(f,n,:)));
    end
end
...
for n = 1:N % Riccati equation solver described in the original paper
    for f = indice%1:F     %size(X(:,:,i))  M*M   T
        TV_1T = squeeze(T_NFK(n,f,:)).'*squeeze(V_NKT(n,:,:));   %ZTV: 1 * T % size(T(i,:)) 1*K; size(Z(:,n)')  1*K size(T(i,:).*Z(:,n)')  1*K 
        % size(V) K*T
        A = reshape(Y(:,:,f)*TV_1T', [M, M]); % (19) of [2] size(Y(:,:,i))  MM x T 
        B = reshape(X(:,:,f)*TV_1T', [M, M]); % (20) of [2] size(X(:,:,i))  MM x T 
