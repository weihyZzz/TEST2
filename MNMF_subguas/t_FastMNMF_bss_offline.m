function [sep,label] = t_FastMNMF_bss_offline(mix,ns,option)
% function [sep,label,alphaend] = t_FastMNMF_bss_offline(mix,ns,option)
% ns: source_num
% M : mic_num
% I : frequence bin
% J : time-frame bin
...
%% offline FastMNMF
    [Xhat,T,V,G,Q,~] = t_FastMNMF_offline(X,XX,ns,MNMF_nb,MNMF_it,MNMF_drawConv,v,trial);
%     [Xhat,T,V,G,Q,alphaend] = t_FastMNMF_offline(X,XX,ns,MNMF_nb,MNMF_it,MNMF_drawConv,v,trial);
%%% qxº∆À„
QX = zeros(I,J,M);
for ii = 1:I
