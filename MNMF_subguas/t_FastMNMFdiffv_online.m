function [Yhat] = t_FastMNMFdiffv_online(X,XX,N,K,maxIt,drawConv,v,trial,batch_size,batch1_size,rho,scalelap,G_FNM,T_NFK,V_NKT,Q_NFMM)
...
%             fprintf('\b\b\b\b%4d', it);
            ...
            local_iterativeUpdate( X_hat,XX_hat, Yhat_FTM, Tj_NFK, Vj_NKT, Gj_FNM, F, Jj, K, N, M, Qj_NFMM, v, Tjhat, Gjhat, Tnumehat, Tdenohat, Gnumehat, Gdenohat, rho );    
        end
    QX = zeros(N,F,Jj,M);
    for ns = 1:N
    for ii = 1:F
...
fprintf(' tFastMNMFdiffv-online done.\n');
end
...
%%% Iterative update %%%
function [ Yhat_FTM, T_NFK, V_NKT, G_FNM, Q_NFMM, Tnume, Tdeno, Gnume, Gdeno] = local_iterativeUpdate( X, XX, Yhat_FTM, T_NFK, V_NKT, G_FNM, F, T, K, N, M, Q_NFMM, v, That, Ghat, Tnumehat, Tdenohat, Gnumehat, Gdenohat, rho )
...
% alpha = (2 * M + v)./(v + 2 * QX_power_Xhat);% F * T (24) in [2]
for n=1:size(v,2) 
    QX_power_Xhat(n,:,:) = squeeze(sum(squeeze(QX_power(n,:,:,:))./ Yhat_FTM,3));% (24) in [2]分母里 QX_power/Xhat, 源1用t分布QX_power(1,:,:)
    alpha(n,:,:) = (2 * M + v(n))./(v(n) + 2 * QX_power_Xhat(n,:,:));% F * T (24) in [2]
    QY_NFTM(n,:,:,:) = (squeeze(QX_power(n,:,:,:))./(Yhat_FTM).^2);
end
% QY_FTM = (QX_power./(Yhat_FTM).^2);
[Tnume ,Tdeno] = local_Tfrac( alpha, QY_NFTM, Yhat_FTM, V_NKT, G_FNM, F, T, K, M ,N); % F x K         % % (30) of [2]分子更新
% T_NFK = T_NFK.*max(sqrt(Tnume./Tdeno),eps); % (30) of [2] 
...
% QX_power_Xhat = squeeze(sum(QX_power./ Yhat_FTM,3));% (24) in [2]分母里 QX_power/Xhat
% alpha = (2 * M + v)./(v + 2 * QX_power_Xhat);% F * T (24) in [2]
% for i=1:size(v,2) alpha(i,:,:) = (2 * M + v(i))./(v(i) + 2 * QX_power_Xhat);end% F * T (24) in [2]
for n=1:size(v,2) 
    QX_power_Xhat(n,:,:) = squeeze(sum(squeeze(QX_power(n,:,:,:))./ Yhat_FTM,3));% (24) in [2]分母里 QX_power/Xhat, 源1用t分布QX_power(1,:,:)
    alpha(n,:,:) = (2 * M + v(n))./(v(n) + 2 * QX_power_Xhat(n,:,:));% F * T (24) in [2]
    QY_NFTM(n,:,:,:) = (squeeze(QX_power(n,:,:,:))./(Yhat_FTM).^2);
...
% QY_FTM = (QX_power./(Yhat_FTM).^2);
[Vnume ,Vdeno] = local_Vfrac( alpha, QY_NFTM, Yhat_FTM, T_NFK, G_FNM, F, T, K, M ,N); % F x K         % % (33) of [2]
V_NKT = V_NKT.*max(sqrt(Vnume./Vdeno),eps); % (33) of [2]
...
% QY_FTM = (QX_power./(Yhat_FTM).^2);
[Gnume ,Gdeno] = local_Gfrac( alpha, QY_NFTM, Yhat_FTM, T_NFK, V_NKT, F, T, M ,N, K); % F x K         % % (35) of [2]
...
V_NFMMM  = local_V_FMMM( alpha, Q_NFMM, XX, Yhat_FTM, F, T, M, N);
...
%%% Tfrac %%%
function [ Tnume, Tdeno ] = local_Tfrac( alpha, QY_NFTM,Yhat_FTM, V_NKT, G_FNM, F, T, K, M ,N)
...
% end
for n=1:N alphaQY_NFTM(n,:,:,:) = squeeze(alpha(n,:,:)).*  squeeze(QY_NFTM(n,:,:,:));end% N * F * T * M
for n = 1:N
    for f = 1:F
...
%%% Vfrac %%%
function [ Vnume, Vdeno ] = local_Vfrac( alpha, QY_NFTM,Yhat_FTM, T_NFK, G_FNM, F, T, K, M ,N) % QY_IJM =|q*x|^2 * y.^-2 即是（31）of [2]除了g的部分
...
% end
% for i=1:size(alpha,1) alphaQY_NFTM(i,:,:,:) = squeeze(alpha(i,:,:)).*  QY_NFTM;end% N * F * T * M
for n=1:N alphaQY_NFTM(n,:,:,:) = squeeze(alpha(n,:,:)).*  squeeze(QY_NFTM(n,:,:,:));end% N * F * T * M
for n = 1:N
    for f = 1:F
    GQY_NFT(n,f,:) = squeeze(G_FNM(f,n,:)).' * squeeze(alphaQY_NFTM(n,f,:,:)).';% 分子中的 alpha * beta (30) of [2] size(G)  % F * N * M 
...
%%% Gfrac %%% 
function [ Gnume, Gdeno ] = local_Gfrac( alpha, QY_NFTM, Yhat_FTM, T_NFK, V_NKT, F, T, M ,N, K) % QY_IJM =|q*x|^2 * y.^-2 即是（31）of [2]除了g的部分
...
%%%% V_FMMM %%%
function [V_NFMMM]  = local_V_FMMM( alpha, Q_NFMM, XX, Yhat_FTM, F, T, M, N)
V_NFTMMM = zeros(N,F,T,M,M,M);%alphaXX = alpha .* XX ;
% alphaXX = squeeze(mean(alpha,1)) .* XX ;%对不同自由度系数取均值mean(alpha,1)alpha(2,:,:)
for n=1:N
for m = 1:M
    V_NFTMMM(n,:,:,m,:,:) = (squeeze(alpha(n,:,:)).* XX)./ Yhat_FTM(:,:,m);% (22) of [2] 
end
end
% V_NFMMM = squeeze(mean(V_NFTMMM,2));
V_NFMMM = squeeze(mean(V_NFTMMM,3));end
...
for n=1:N
for f = 1:F
    QX_power(n,f,:,:) = abs(squeeze(X(f,:,:)) * squeeze(Q_NFMM(n,f,:,:)).').^2;% N * F * T* M
end
end
end
