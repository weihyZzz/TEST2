function [Yhat_FTM,T_NFK,V_NKT,G_FNM,Q_NFMM,cost] = mpdf_FastMNMF_offline(X,XX,N,K,maxIt,drawConv,v,trial,pro,G_FNM,T_NFK,V_NKT,Q_NFMM)
...
%% main iterate
    for it = 1:maxIt
        fprintf('\b\b\b\b%4d', it);
        [ Yhat_FTM, T_NFK, V_NKT, G_FNM, Q_NFMM ] = local_iterativeUpdate(X, XX, Yhat_FTM, T_NFK, V_NKT, G_FNM, F, T, K, N, M , Q_NFMM, v, pro);
    end
end

fprintf(' mpdfFastMNMF done.\n');
end
...
%%% Iterative update %%%
function [ Yhat_FTM, T_NFK, V_NKT, G_FNM, Q_NFMM] = local_iterativeUpdate(X, XX, Yhat_FTM, T_NFK, V_NKT, G_FNM, F, T, K, N, M ,Q_NFMM,v, pro)

%%%%% Update T %%%%%% F * K 
QX_power = local_QX_power( Q_NFMM, X, F, T, M, N);% N F T M
% QX_power_Xhat = squeeze(sum(QX_power./ Yhat_FTM,3));% (24) in [2]分母里 QX_power/Xhat
QX_power_Xhat = squeeze(sum(squeeze(QX_power(1,:,:,:))./ Yhat_FTM,3));% (24) in [2]分母里 QX_power/Xhat, 源1用t分布QX_power(1,:,:)
alpha = (2 * M + v)./(v + 2 * QX_power_Xhat);% F * T (24) in [2]
% QY_FTM = (QX_power./(Yhat_FTM).^2);
for n=1:N QY_NFTM(n,:,:,:) = (squeeze(QX_power(n,:,:,:))./(Yhat_FTM).^2);end
% [Tnume ,Tdeno] = local_Tfrac( alpha, QY_FTM, Yhat_FTM, V_NKT, G_FNM, F, T, K, M ,N); % F x K         % % (30) of [2]分子更新
...
% if N == 2
% % 针对源1,2的不同分布。1使用t分布，2使用高斯分布
% alphaQY_NFTM(1,:,:,:) = alpha.*  squeeze(QY_NFTM(1,:,:,:));% N * F * T * M
% alphaQY_NFTM(2,:,:,:) = squeeze(QY_NFTM(2,:,:,:));% N * F * T * M
% elseif N == 3
% alphaQY_NFTM(1,:,:,:) = alpha.*  squeeze(QY_NFTM(1,:,:,:));% N * F * T * M
% alphaQY_NFTM(2,:,:,:) = squeeze(QY_NFTM(2,:,:,:));% N * F * T * M
% alphaQY_NFTM(3,:,:,:) = alpha.*  squeeze(QY_NFTM(1,:,:,:));% N * F * T * M  
% elseif N == 4
% alphaQY_NFTM(1,:,:,:) = alpha.*  squeeze(QY_NFTM(1,:,:,:));% N * F * T * M
% alphaQY_NFTM(2,:,:,:) = squeeze(QY_NFTM(2,:,:,:));% N * F * T * M
% alphaQY_NFTM(3,:,:,:) = alpha.*  squeeze(QY_NFTM(1,:,:,:));% N * F * T * M
% alphaQY_NFTM(4,:,:,:) = squeeze(QY_NFTM(2,:,:,:));% N * F * T * M        
% end
for n = 1:N
    for f = 1:F
...
for m = 1:M
    ...
    %%还没改~！！！！！！！！！！！！！！！！！！！！！
%     if N == 2
%         V_NFTMMM(1,:,:,m,:,:) = alphaXX./ Yhat_FTM(:,:,m);% (22) of [2] 
%         V_NFTMMM(2,:,:,m,:,:) = XX./ Yhat_FTM(:,:,m);% (22) of [2] 
%     elseif N == 3
%         V_NFTMMM(1,:,:,m,:,:) = alphaXX./ Yhat_FTM(:,:,m);% (22) of [2] 
%         V_NFTMMM(2,:,:,m,:,:) = XX./ Yhat_FTM(:,:,m);% (22) of [2] 
%         V_NFTMMM(3,:,:,m,:,:) = alphaXX./ Yhat_FTM(:,:,m);% (22) of [2] 
%     elseif N == 4
%         V_NFTMMM(1,:,:,m,:,:) = alphaXX./ Yhat_FTM(:,:,m);% (22) of [2] 
%         V_NFTMMM(2,:,:,m,:,:) = XX./ Yhat_FTM(:,:,m);% (22) of [2] 
%         V_NFTMMM(3,:,:,m,:,:) = alphaXX./ Yhat_FTM(:,:,m);% (22) of [2] 
%         V_NFTMMM(4,:,:,m,:,:) = XX./ Yhat_FTM(:,:,m);% (22) of [2] 
%     end
end
