function [Yhat] = t_FastMNMF_online(X,XX,N,K,maxIt,drawConv,v,trial,batch_size,batch1_size,rho,scalelap,G_FNM,T_NFK,V_NKT,Q_FMM)
...
%     V_NKT = max(gamrnd(shape,power_observation*F*M/(shape*N*K),[N K batch_size]),eps);%(79) of [4]第二个参数为逆尺度参数，是文章中的倒数与python版本一致
...
if sum(size(T_NFK) ~= [N,F,K]) || sum(size(V_NKT) ~= [N,K,batch_size]) || sum(size(G_FNM) ~= [F,N,M]) %|| sum(size(Z) ~= [K,N])
...
fprintf('Iteration:    ');
    for it = 1:trial
        ...
        [Q_FMM, G_FNM, T1, V1] = normalize(Q_FMM, G_FNM, T_NFK, V1, M);
        Yhat_FTM = local_Yhat( T_NFK, V1, G_FNM, F, batch1_size, M, N); % (17) of [2] initial model tensor F *M *M 
        [ ~, T1, V1] = init_local_iterativeUpdate(X(:,1:batch1_size,:), XX, Yhat_FTM, T1, V1, G_FNM, F, batch1_size, K, N, M ,Q_FMM, v);
    end
...
%     end
    ...
    for j = batch1_size:T
        ...
        if j == batch1_size
            ...
            Gjhat = zeros(F,N,M); Gnumehat = zeros(F,N,M); Gdenohat = zeros(F,N,M);Qj_FMM = Q_FMM;
...
%             fprintf('\b\b\b\b%4d', it);
            ...
            local_iterativeUpdate( X_hat,XX_hat, Yhat_FTM, Tj_NFK, Vj_NKT, Gj_FNM, F, Jj, K, N, M, Qj_FMM, v, Tjhat, Gjhat, Tnumehat, Tdenohat, Gnumehat, Gdenohat, rho );    
        end
    QX = zeros(F,Jj,M);
    for ii = 1:F
...
%                         Yhat(f,t,:,src) = ys * squeeze(G(f,src,:,:))/Yhat_IJM(:,:,f,t)*X(:,f,t); % (54) of [2] M x 1       
                        Yhat(f,t,:,src) = inv(squeeze(Qj_FMM(f,:,:)))*diag(ys * squeeze(Gj_FNM(f,src,:))./( squeeze(Yhat_FTM(f,t,:)) +eps))* squeeze(QX(f,t,:));%squeeze(Q(i,:,:)) *x(:,i,j); % M x 1 (19) of [2]
                    end              
                end          
            end
        else
            for f = 1:F    
               for src = 1:N                 
                    ...
                    Yhat(f,j,:,src) = inv(squeeze(Qj_FMM(f,:,:)))*diag(ys * squeeze(Gj_FNM(f,src,:))./( squeeze(Yhat_FTM(f,batch_size,:)) +eps)) * squeeze(QX(f,batch_size,:));%squeeze(Q(i,:,:)) *x(:,i,j); % M x 1 (19) of [2]
               end                      
            end
        end
    end
...
[Tnume ,Tdeno] = local_Tfrac( alpha, QY_FTM, Yhat_FTM, V_NKT, G_FNM, F, T, K, M ,N); % F x K         % % (20) of [2]分子更新
T_NFK = T_NFK.*max(sqrt(Tnume./Tdeno),eps); % (20) of [2] 
...
%%% Iterative update %%%
function [ Yhat_FTM, T_NFK, V_NKT, G_FNM, Q_FMM, Tnume, Tdeno, Gnume, Gdeno] = local_iterativeUpdate( X, XX, Yhat_FTM, T_NFK, V_NKT, G_FNM, F, T, K, N, M, Q_FMM, v, That, Ghat, Tnumehat, Tdenohat, Gnumehat, Gdenohat, rho )
...
G_FNM = sqrt(Fjaxa(G_FNM, Gnume, Ghat, Gnumehat, rho) ./ Fjx(Gdeno, Gdenohat, rho))+ 1e-10; % (15) of [2]
...
G_FNM = G_FNM./G_sum ;
T_NFK = T_NFK .* squeeze(sum(G_sum,2)).';%sum(G_sum,2);% N * F * K  
 
T_sum = sum(T_NFK,2);% N *1* K
T_NFK = T_NFK./T_sum;
V_NKT = V_NKT .* squeeze(T_sum);
end

%%% Tfrac %%%
function [ Tnume, Tdeno ] = local_Tfrac( alpha, QY_FTM,Yhat_FTM, V_NKT, G_FNM, F, T, K, M ,N)
GQY_NFT = zeros(N,F,T);GYhat_NFTM = zeros(N,F,T,M);Tnume = zeros(N,F,K);Tdeno = zeros(N,F,K); 
alphaQY_FTM = alpha.*  QY_FTM;% F * T * M
for f = 1:F
    GQY_NFT(:,f,:) = squeeze(G_FNM(f,:,:)) * squeeze(alphaQY_FTM(f,:,:)).';%(31) of [2] size(G)  % F * N * M 
end
for n=1:N
    for t=1:T
    GYhat_NFTM(n,:,t,:) = squeeze(G_FNM(:,n,:)) ./ squeeze(Yhat_FTM(:,t,:));    
    end
end
GYhat_NFT = squeeze(sum(GYhat_NFTM,4));
for n=1:N
    Tnume(n,:,:) = squeeze(GQY_NFT(n,:,:)) * squeeze(V_NKT(n,:,:)).'; 
    Tdeno(n,:,:) = squeeze(GYhat_NFT(n,:,:)) * squeeze(V_NKT(n,:,:)).'; 
end
end 

%%% Vfrac %%%
function [ Vnume, Vdeno ] = local_Vfrac( alpha, QY_FTM,Yhat_FTM, T_NFK, G_FNM, F, T, K, M ,N)
GQY_NFT = zeros(N,F,T);GYhat_NFTM = zeros(N,F,T,M);Vnume = zeros(N,K,T);Vdeno = zeros(N,K,T); %size(XX)  % F * T * M 
alphaQY_FTM = alpha.*  QY_FTM;% F * T * M
for f = 1:F
    GQY_NFT(:,f,:) = squeeze(G_FNM(f,:,:)) * squeeze(alphaQY_FTM(f,:,:)).';
end
for n=1:N
    for j=1:T
    GYhat_NFTM(n,:,j,:) = squeeze(G_FNM(:,n,:)) ./ squeeze(Yhat_FTM(:,j,:));    
    end
end
GYhat_NFT = squeeze(sum(GYhat_NFTM,4));
for n=1:N
    Vnume(n,:,:) = squeeze(T_NFK(n,:,:)).' * squeeze(GQY_NFT(n,:,:)); 
    Vdeno(n,:,:) = squeeze(T_NFK(n,:,:)).' * squeeze(GYhat_NFT(n,:,:)); 
end
end

% %%% Zfrac %%%
% function [ Znume, Zdeno ] = local_Zfrac( alpha, QY_IJM,Yhat_IJM, T, V, G, F, T, K, M ,N)
...
%%% Gfrac %%% 
function [ Gnume, Gdeno ] = local_Gfrac( alpha, QY_FTM, Yhat_FTM, T_NFK, V_NKT, F, T, M ,N, K)
...
for f = 1:F
    Gnume(f,:,:) = squeeze(TV_NFT(:,f,:)) * squeeze(alphaQY_FTM(f,:,:));
end
for n = 1:N
    for m = 1:M
    Gdeno_NFTM(n,:,:,m) = squeeze(TV_NFT(n,:,:)) ./ squeeze(Yhat_FTM(:,:,m));    
    end
end
