...
%% offline subguass Multichannel NMF
    [Xhat,T,V,G,Q,~] = subguas_FastMNMF_offline(X,XX,ns,MNMF_nb,MNMF_it,MNMF_drawConv,option);
...
% Xhat_hat = local_Xhat( T, V, G, Z, I, J, M,Q);% I* J* M* M
%%
for i = 1:I
    for j = 1:J
        for src = 1:ns
...
%           G_hat(i,src,:,:) = inv(squeeze(Q(i,:,:))) *diag(squeeze(G(i,src,:)))*inv(squeeze(Q(i,:,:))');
            %           Y(i,j,:,src) = inv(squeeze(Q(i,:,:)))* ys * squeeze(G(i,src,:))./(squeeze(Xhat(i,j,:))+eps).*squeeze(QX(i,j,:)); % M x 1
%           Y(i,j,:,src) = ys * squeeze(G_hat(i,src,:,:))/squeeze(Xhat_hat(i,j,:,:))*squeeze(QX(i,j,:)); % M x 1
            Y(i,j,:,src) = inv(squeeze(Q(i,:,:)))*diag(ys * squeeze(G(i,src,:))./( squeeze(Xhat(i,j,:)) +eps))* squeeze(Q(i,:,:)) *x(:,i,j); % M x 1 (19) of [2]
