function [Yhat_FTM,T_NFK,V_NKT,G_FNM,Q_NFMM,cost] = t_FastMNMFdiffv_offline(X,XX,N,K,maxIt,drawConv,v,trial,G_FNM,T_NFK,V_NKT,Q_NFMM)
...
if ( drawConv == true )
    ...
    cost(1) = local_cost( X, Yhat_FTM, F, T, M ,Q_NFMM); % initial cost value
   for it = 1:trial
        fprintf('\b\b\b\b%4d', it);
        [ Yhat_FTM, T_NFK, V_NKT] = init_local_iterativeUpdate(X, XX, Yhat_FTM, T_NFK, V_NKT, G_FNM, F, T, K, N, M ,Q_NFMM, v);
        cost(it+1) = local_cost( X, Yhat_FTM, F, T, M ,Q_NFMM);
   end
...
fprintf('Iteration:    ');
    ...
    for it = 1:maxIt
        fprintf('\b\b\b\b%4d', it);
        [ Yhat_FTM, T_NFK, V_NKT, G_FNM, Q_NFMM ] = local_iterativeUpdate(X, XX, Yhat_FTM, T_NFK, V_NKT, G_FNM, F, T, K, N, M ,Q_NFMM, v);
        cost(it+1) = local_cost( X, Yhat_FTM, F, T, M ,Q_NFMM);
    end
    figure;
