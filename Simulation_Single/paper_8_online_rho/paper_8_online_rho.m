...
clear all; close all;
addpath('room');addpath('components');addpath('bss_eval');addpath('plot');addpath('EM');addpath('nmfiva');addpath('IVE');addpath('audioSignalProcessTools');addpath('DOA');addpath('MNMF');addpath('CGGMM_IVA');addpath('MAIN_sim_components');addpath('AV-GMM_IVA');addpath('MNMF_subguas');addpath('PEASSevaluate');addpath('MLDR');addpath('bse_SCM');addpath('AR');addpath('GWPE');addpath('MNMF_python');%addpath('PEASS');
filenameTmp =strcat('paper_8_online_rho');%test_em_实录_beta AGC_method 可将仿真参数作为文件名 test_online_offline   test_AWGN_Lb test_AWGN_Lb
...
target_index_case = {[2 5]}; intf_index_case = {[]};
% 1女 2男 3笛子 4吉他 5唤醒词 6洗衣机 [1 2],[1 3],[2 5],[2 4],[5 3],[5 6],[3 4]
...
target_source_num_case = [2];           % 目标源数目，该值<=2，=1时为单源提取
...
reverbTime_case = [0.15];
                                        % 房间混响时间，单位为秒
angle_case = [80];                      % 两源最小夹角,angle from 0 to 360,单位为度。0度角为X轴右方向
angle_start_case = [40];                % 从该位置开始进行源的排布,角度要求同上。>180时，用0度代替，eg.180,225，应用-45,0代替
R_ratio_case = [1];                   % R = 源与麦克风阵列最大距离max_R * R_ratio, 取值from 0 to 1  paper 0.5 0.32
tR_ratio_case = [0.5];                 % 目标源与干扰源之间的距离比值
src_permute_type_case = [0];            % 1—随机源排布，0—顺序源排布
room_plot_on = 2;                       % 是否绘制房间平面图，0—不绘制，1—每次都绘制，2—仅绘制第一次，3—绘制车载环境
room_size_case = {[8 5 3]};            % 房间大小 长宽高
mic_center_case = {[4 0.5 1.2]};          % 麦克风线阵中心坐标 阵列默认高度相同 [4 0.5 1.2] [4 2.5 1.2]
mic_distx_case = [0.0566];               % 相邻麦克距离的水平方向投影长度 正值：x轴正方向 负值：x轴负方向
...
batch_algs_case = [22];                  
% batch IVA的类型,
%1-auxiva,2-MNMF，3-EMIVA，4-IVE ,5-EMIVA_fast，6-AV-GMM-IVA,7-subguassmnmf,8-tmnmf.9-fastmnmf,
%10-t_fastmnmf,11-MLDR;12-fastmnmf2;13-tfastdifferent v ;14-tfast mpdf混合pdf控制本函数内算法; 16-AR_FastMNMF2; 18-AR_t_FastMNMF1; 19-AR_t_FastMNMF2 ;

%17-AR_FastMNMF1; 20-AR_t_FastMNMF1_new ;25-AR_t_FastMNMF2_new
%21-myMNMF; 22-my_FastMNMF; 23-my_FastMNMF2; 24-my_t_FastMNMF
online_algs_case = [2];                 
% online IVA的类型,
%1-auxiva,2-MNMF，3-EMIVA，4-IVE, 5-tmnmf, 6-fastmnmf, 
%7-tfastmnmf, 8-fastmnmf2, 9-MLDR，10-mpdf,11-t-differv,12-subguass;
...
MNMF_nb_case = [30];                    % number of MNMF bases for all sources (total bases) 10:10:50
MNMF_it_case = [20];                    % number of MNMF iterations 100
MNMF_first_batch_case = [20];           % first mini-batch size
MNMF_batch_size_case = [10];            % mini-batch size
MNMF_rho_case = [0.1 0.5 0.9 1.5 3 5 10];                  % the weight of last batch
...
tMNMF_v_case = [4];                     %[8] {[1,8]}是普通t,diffv是用cell;t-distribution 's freedom [0-20]
...
AR_init_it_case = [30];                 % AR init iterations
AR_tap_case = [4];                      % AR tap length
AR_delay_case = [3];                    % AR delay size
AR_init_SCM_case = {'circular'};        % AR init SCM ways {'circular', 'obs','ILRMA'}
AR_internorm_case = [1];                % AR interval norm
...
%% Prior AuxIVA %%%
DOA_switch_case=[0];                    % 控制DOA,0-[0,0,0,0]&doa_0; 1-[1,1,1,0]&doa_1
...
for batch_algs = batch_algs_case   for online_algs = online_algs_case 
...
for MNMF_delta = MNMF_delta_case for MNMF_p_norm = MNMF_p_norm_range for sub_beta = sub_beta_case for tMNMF_v = tMNMF_v_case for tMNMF_trial = tMNMF_trial_case for mpdf_pro = mpdf_probability_case 
for AR_init_it = AR_init_it_case for AR_tap = AR_tap_case for AR_delay = AR_delay_case for AR_init_SCM = AR_init_SCM_case for AR_internorm = AR_internorm_case
for ILRMA_init = ILRMA_init_case for ILRMA_type = ILRMA_type_case for ILRMA_nb = ILRMA_nb_case for ILRMA_it = ILRMA_it_case for ILRMA_normalize = ILRMA_normalize_case for ILRMA_dlratio = ILRMA_dlratio_case
...
%             [s_hat,label] = AR_FastMNMF2(x.',source_num,option);
        ...
        elseif batch_algs == 19 % AR_t_FastMNMF2
            ...
            s_est = s_hat.';
        elseif batch_algs == 20 % AR_t_FastMNMF1_new
            [s_hat,label] = AR_t_FastMNMF_new(x.',source_num,option);
            s_est = s_hat.';
        elseif batch_algs == 21 % MNMF_my
            [s_hat,label] = bss_MNMF_python(x.',source_num,option);
            s_est = squeeze(s_hat(:,MNMF_refMic,:)).';
        elseif batch_algs == 22 % my_FastMNMF
            [s_hat,label] = FastMNMF_my(x.',source_num,option);
            s_est = s_hat.';
        elseif batch_algs == 23 % my_FastMNMF2
            [s_hat,label] = FastMNMF2_my(x.',source_num,option);
            s_est = s_hat.';
        elseif batch_algs == 24 % my_t_FastMNMF
            [s_hat,label] = t_FastMNMF_my(x.',source_num,option);
            s_est = s_hat.';
        elseif batch_algs == 25 % AR_t_FastMNMF2_new
            [s_hat,label] = AR_t_FastMNMF2_new(x.',source_num,option);
            s_est = s_hat.';
        end
        out_type = 'batch';
    end 
...
%             cost_case = [cost_case cost];
        end
    end
    fid=fopen('paper_record.txt','a');fprintf(fid,'\n%s\n',filenameTmp);fprintf(fid,'SDR = ');fprintf(fid,'%g   ',SDR);
    fprintf(fid,'SIR = ');fprintf(fid,'%g   ',SIR);
%     fprintf(fid,'cost = ');fprintf(fid,'%g   ',cost);
    fclose(fid);
      %% 保存数据(case)和音频
...
%         case_name = {'女+男30','女+男50','女+笛30','女+笛50','男+唤30','男+唤50','男+吉30',...
%             '男+吉50','唤+笛30','唤+笛50','唤+洗30','唤+洗50','笛+吉30','笛+吉50'};
%         case_name = {'女声+男声','女声+笛子','男声+唤醒词','男声+吉他','唤醒词+笛子','唤醒词+洗衣机','笛子+吉他',};        
        case_name={'FastMNMF','FastMNMF2','t-FastMNMF','AR-FastMNMF','AR-t-FastMNMF','AR-t-FastMNMF2','FastMNMF','FastMNMF2','t-FastMNMF','AR-FastMNMF','AR-t-FastMNMF','AR-t-FastMNMF2'};        
%         SDR_total = reshape(SDR_case_full,R_num, total_iter_num, case_num1);
%        case_name = {'tap=2 Δ=1','tap=2 Δ=3','tap=2 Δ=5','tap=4 Δ=1','tap=4 Δ=3','tap=4 Δ=5','tap=8 Δ=1','tap=8 Δ=3','tap=8 Δ=5'...
%            'tap=2 Δ=1','tap=2 Δ=3','tap=2 Δ=5','tap=4 Δ=1','tap=4 Δ=3','tap=4 Δ=5','tap=8 Δ=1','tap=8 Δ=3','tap=8 Δ=5'...
%            'tap=2 Δ=1','tap=2 Δ=3','tap=2 Δ=5','tap=4 Δ=1','tap=4 Δ=3','tap=4 Δ=5','tap=8 Δ=1','tap=8 Δ=3','tap=8 Δ=5'};%输入算法名称 不输入名称则输出case1,case2,case3
...
%          case_num2 = size(SDR_time_all_full,1) / 2;
         ...
         SortedPlotThr = 15; % >1,表明显示排序。
         SortedPlotNum = 7; % 表明排序显示case的数目,在 sub_case_num 比较大的时候可以清晰显示
...
%          plotSIR_time1(SDR_time_all_full,T,case_num2,sub_case_num,packFigNum,SortedPlotThr,SortedPlotNum,plotRatio); %,plot_case 绘制时变SIR
         plotSIR_time_paper(SIR_time_all,T,case_num,sub_case_num,packFigNum,SortedPlotThr,SortedPlotNum,plotRatio,Lb_case); %,plot_case 绘制时变SIR
...
%             case_num = size(SDR_case_full,1) / total_iter_num;
            ...
            case_num = size(SDR_case,2) / total_iter_num;
            sub_case_num = case_num/2;
            packFigNum =   2;  % 一个fig 分成几个(=packFigNum)subfugure，进行情况比较
            SortedPlotThr = 30; % sub_case_num * packFigNum < SortedPlotThr不排序
            SortedPlotNum = 6; % 表明排序显示case的数目,在 sub_case_num 比较大的时候可以清晰显示
            plotRatio =1;   %一次多少比例的图, default=1, 2 表明画50% 的图；
...
%             end
            ...
            plotSDR_errorbar(SDR_total,case_num);
        end 
%          end
    end
%     close all;
