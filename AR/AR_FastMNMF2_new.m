function [sep, label] = AR_FastMNMF2_new(mix, N, option)
%%% AR_FastMNMF2 %%%
% Reference:
...
for it = start_idx : n_iter
    ...
    for n = 1 : N 
        tmp1_NFT(n, :, :) = squeeze(sum(permute(Qd_power_FTM ./ (Y_FTM .^ 2), [3, 1, 2]) .* (G_NM(n, :).'), 1));
    ...
    for n = 1 : N
        G_numerator(n, :) = squeeze(sum(squeeze(PSD_NFT(n, :, :)) .* (Qd_power_FTM ./ (Y_FTM .^ 2)), [1 2])); % numerator of (15) in [1]
