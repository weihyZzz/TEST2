...
Xbar_FxTxMLa = zeros(F, T, M * (n_tap_AR + 1)); % size: F x T x M*(tap+1) 相比FastMNMF多存tap个X_FTM
...
for it = start_idx : n_iter
    ...
    for m = 1 : M
        ...
        for f = 1 : F
            ...
            u_M = tmp_MM(:, m);
            denominator  = sqrt(u_M' * Vinv_MLtxMLt(1 : M, 1 : M) * u_M); % 求V公式10的分子部分共轭转置是自身
            P_FxMxMLa(f, m, :) = conj((Vinv_MLtxMLt(:, 1 : M) * u_M) ./ denominator); % (11)(12) in [1] 取共轭是因为求V公式10的分母y_ftm，在计算公式12时需要做一次复共轭
        end
        Q_FMM = P_FxMxMLa(:, :, 1 : M);
    end
...
% P_FxMxMLa and Xbar_FxTxMLa 包含L帧之前信号
[F, M, ~] = size(P_FxMxMLa);
[~, T, ~] = size(Xbar_FxTxMLa);
Px_FTM = zeros(F, T, M);
for f = 1 : F
