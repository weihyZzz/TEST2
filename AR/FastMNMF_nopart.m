function [sep, label] = FastMNMF_nopart(mix, N, option)
%% FastMNMF_nopart.m %%
%  FastMNMF without partition z version
%% init parameter
g_eps = 5e-2;
EPS = 1e-10;
...
switch init_SCM
    ...
    case 'obs'
        ...
        for f = 1 : F
            ...
            XX_FMM(f, :, :) = tmp_MM;
            [vec, val] = eig(tmp_MM);
%             [eig_val, index] = sort(diag(val), 'descend'); % descend降序？不是降序仿真得到了
...
for it = start_idx : n_iter
    ...
    for n = 1 : N 
        tmp1_NFT(n, :, :) = squeeze(sum(permute(Qx_power_FTM ./ (Y_FTM .^ 2), [3, 1, 2]) .* permute(G_NFM(n, :, :), [3, 2, 1]), 1));
    ...
    for n = 1 : N
        G_numerator(n, :, :) = squeeze(sum(squeeze(PSD_NFT(n, :, :)) .* (Qx_power_FTM ./ (Y_FTM .^ 2)), 2)); % numerator of (15) in [1]
