function [sep, label] = AR_t_FastMNMF(mix, N, option)
%%% AR_t_FastMNMF %%%
% Reference:
...
%
%    The joint blind souce separation and dereverberation method that integrates t_FastMNMF with the AR reverberation model.
%
...
for it = start_idx : n_iter
    ...
    for n = 1 : N 
        tmp1_NFT(n, :, :) = squeeze(sum(permute(Qd_power_FTM ./ (Y_FTM .^ 2) .* alpha_FT, [3, 1, 2]) .* permute(G_NFM(n, :, :), [3, 2, 1]), 1));
    ...
    for n = 1 : N
        G_numerator(n, :, :) = squeeze(sum(squeeze(PSD_NFT(n, :, :)) .* alpha_FT .* (Qd_power_FTM ./ (Y_FTM .^ 2)), 2)); % numerator of (15) in [1]
    ...
    alpha_FT = (2 * M + v) ./ (v + 2 * sum(Qd_power_FTM ./ Y_FTM, 3)); % (24) in [3]
    for m = 1 : M
        tmp_FxMxM = squeeze(mean(DD_FxTxMxM .* alpha_FT ./ Y_FTM(:, :, m), 2)); % (22) in [3]
        for f = 1 : F
