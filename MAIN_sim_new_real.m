...
clear all; close all;
addpath('room');addpath('components');addpath('bss_eval');addpath('plot');addpath('EM');addpath('nmfiva');addpath('IVE');addpath('audioSignalProcessTools');addpath('DOA');addpath('MNMF');addpath('CGGMM_IVA');addpath('MAIN_sim_components');addpath('oppo');addpath('samples');addpath('MNMF_subguas');addpath('MNMF');
mixfile = strcat('samples');%almethod = strcat('_em_batch_7个文件');
% path1 = strcat('D:\github-code\IVATEST\',mixfile);t=0;% 记录符合条件的文件数量t
source_path = 'samples';t=0;% 记录符合条件的文件数量t
filestr1 = dir([source_path,'.']); % 读信号文件夹
for fnum1 = 3 :size(filestr1) % 遍历四源任选两组合的六种情况,jovi&man&woman&xvxv
    source_path2nd = strcat(source_path,'\',filestr1(fnum1).name,'\');SDR_case_full=[];SDR_time_all_full=[];% 为了一起画出遍历两个源混合的六种情况SDR
    filestr2 = dir([source_path2nd,'*.wav']); % 读信号文件
for fnum2 = 1 :size(filestr2,1) % 遍历两个源混合的六种情况(实录混合信号，mix>jovi&man，...)
    filenameTmp = filestr2(fnum2).name; 
%     filenameTmp =strcat('jovi_man_',num2str(files));%test_em_实录_beta AGC_method 可将仿真参数作为文件名 test_online_offline   test_AWGN_Lb test_AWGN_Lb
mkdir_str=strcat('./Simulation_Single_mpdf_diff_v/',filestr1(fnum1).name,'/',filenameTmp(1:end-4));
...
%                    6—洗衣机, 7—音乐, 8—高斯白噪声, 9-jovi, 10-man, 11-woman, 12-xvxv}
...
mix_SINR_case = [0];                    % 目标信号和噪声（或者干扰信号）的混合信干噪比，若mix_SINR=0，则为1:1混合，仅在normal_case=3时;使用以dB为单位，表示 target/(interference+diffuse_noise)
...
room_plot_on = 0;                       % 是否绘制房间平面图，0—不绘制，1—每次都绘制，2—仅绘制第一次
...
%% Iteration num %%
iter_num_case = [2];                    % DOA+20次？，算法迭代次数，online 固定为2次，offline的值为case的值 default=20
...
% batch_algs_case = [9];                  % batch IVA的类型,1-auxiva,2-MNMF，3-EMIVA，4-IVE ，控制本函数内算法,5-EMIVA_fast
% online_algs_case = [6];                 % online IVA的类型,1-auxiva,2-MNMF，3-EMIVA，4-IVE
batch_algs_case = [14];                  % batch IVA的类型,1-auxiva,2-MNMF，3-EMIVA，4-IVE ,5-EMIVA_fast，6-AV-GMM-IVA,7-subguassmnmf,8-tmnmf.9-fastmnmf,10-t_fastmnmf,11-MLDR;12-fastmnmf2;13-tfastdifferent v ;14-tfast mpdf混合pdf控制本函数内算法
online_algs_case = [10];                 % online IVA的类型,1-auxiva,2-MNMF，3-EMIVA，4-IVE, 5-tmnmf, 6-fastmnmf, 7-tfastmnmf, 8-fastmnmf2, 9-MLDR，10-mpdf,11-t-differv;
...
MNMF_nb_case = [40];                    % number of NMF bases for all sources (total bases)
...
MNMF_shiftSize_case = [1024];           % shift length in STFT [points]
...
tMNMF_v_case = [2,4,8,10,15,20];                            % t-distribution 's freedom [0-20]
...
% prior_case = {[1,1,1,0]};    SIR_time_all_full         % 输入为(1,k)维向量，第k项为1代表有第k个源的先验信息，
...
DOA_Null_ratio_range = [1000]/1000;      % DOA  加权值，[0.1]/10-4mic, 
...
for MNMF_delta = MNMF_delta_case for MNMF_p_norm = MNMF_p_norm_range  for tMNMF_v = tMNMF_v_case for tMNMF_trial = tMNMF_trial_case for ILRMA_init = ILRMA_init_case for ILRMA_type = ILRMA_type_case for ILRMA_nb = ILRMA_nb_case for ILRMA_it = ILRMA_it_case for ILRMA_normalize = ILRMA_normalize_case for ILRMA_dlratio = ILRMA_dlratio_case
...
%           [s_est,label] = nmfiva_audio_bss_online_perm(x,source_num,option);              
        ...
        elseif online_algs == 7 % t_fastmnmf 
            [s_hat,label] = t_FastMNMF2_bss_online(x.',source_num,option); 
             s_est = squeeze(s_hat(:,MNMF_refMic,:)).';
        ...
        elseif online_algs == 10 % tfastmnmf differnt pdf; 
...
%             [pcaData,projectionVectors,eigVal] = whitening_pre(x',2);
        ...
        elseif batch_algs == 10 % t_fastmnmf 
            [s_hat,label] = t_FastMNMF2_offline(x.',source_num,option); 
             s_est = squeeze(s_hat(:,MNMF_refMic,:)).';
...
%     sav=['save ' strsave]; eval(sav);
    ...
    if real_result == 1 %% 按照sep1_method1方式命名     'case_',case_str,
      audiowrite([sound_dir,'/',filenameTmp(:,1:end-4),'_out1ch_case_',case_str,'.wav'], s_est(1,:)', fs_ref);
      if size(s_est,1) == 2
        audiowrite([sound_dir,'/',filenameTmp(:,1:end-4),'_out2ch_case_',case_str,'.wav'], s_est(2,:)', fs_ref);
      end
...
%       end
      if size(s_est,1) == 3
          audiowrite([sound_dir,'/',filenameTmp(:,1:end-4),'_out2ch_case_',case_str,'.wav'], s_est(2,:)', fs_ref);
          audiowrite([sound_dir,'/',filenameTmp(:,1:end-4),'_out3ch_case_',case_str,'.wav'], s_est(3,:)', fs_ref);
      end
      if size(s_est,1) == 4
...
disp(['符合条件的文件有',num2str(t),'个']);
% %% 处理文件
% outputname = almethod(5:end);%'oppo分离数据11-6';% 按需改变
% path1 = strcat('D:\github-code\IVATEST\Simulation_Single\',mixfile,almethod);% 按需改变
% filestr1 = dir([path1,'.']);
% for fnum1 = 1+2 :size(filestr1)
%     path2 = strcat(path1,'\',filestr1(fnum1).name,'\');% 一级目录文件夹
%     filestr2 = dir([path2,'.']);% 一级目录文件夹数据读取
...
%     mkdir_str=strcat('.\',outputname,'\',filestr2(fnum2).name);%filestr3(1:end-4)去掉后缀.wav
%     mkdir(mkdir_str);% 一运行就会在当前文件夹下创建对应文件名的文件夹       
