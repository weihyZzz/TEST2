function [Y, W, D1, D2, W1_f, H1_f, W2_f, H2_f, V,obj_vals] = ...
        binaural_nmfiva_online_update(X, W, V, D1, D2, W1_f, H1_f, W2_f, H2_f, Gr, Fr, varargin)
% NMF IVA
...
%     global seed; seed = rng(20);
    ...
    for iter = 1:iter_num       
        ...
        for n = 1:N
            ...
            V1 = reshape(V1, 2, 2); 
            V1 = alpha * V1 + (1-alpha) * squeeze(V(n,:,:,1));            
            ...
            V2 = reshape(V2, 2, 2);
            V2 = alpha * V2 + (1-alpha) * squeeze(V(n,:,:,2));
        ...
        W1_f_old = W1_f;
        H1_f_old = H1_f;
        [W1_f, H1_f] = nmf_online(abs(Y(:,:,1)).^2 .* Fr{1}(R1)', ...
                           W1_f, H1_f, nmf_option);
        W2_f_old = W2_f;
        H2_f_old = H2_f;
        [W2_f, H2_f] = nmf_online(abs(Y(:,:,2)).^2 .* Fr{2}(R2)', ...
        ...
        if ~verbose
            obj_vals(iter+1) = obj(W, D1, D2, ...
                W_old, D1_old, D2_old, X, Gr, Fr);
...
function E = auxfun_min_sol(V1, V2)
    ...
    E = E * diag(diag(E \ eye(2))); % E * diag of E^(-1), what does it mean? 
    % W = E';
