function [sep,label] = tbse_SCM_offline(mix,ns,option)
...
% ILRMA initialization BSE利用ILRMA迭代后的结果：基和解混合矩阵作为初始化参数
...
for i = 1:I
    for j = 1:J
        for src = 1:ns           
            for k = 1:MNMF_nb
                rt(i,j) = rt(i,j) + Z(src,k)*T(i,k)*V(k,j);% (18) of [1]
            end
        end%size(rt)  I x J 
    end
end
...
Rn_hat = squeeze(1/J * sum(Rn_tmp,4));% M x M x I   
  % rn
  rn = zeros(I,J);
for i = 1:I
    for j = 1:J
        rn(i,j)=1/M * y_hat(:,i,j)' * pinv(Rn_tmp(:,:,i,j)) * y_hat(:,i,j);% (19) of [1]
    end
end  

%% offline SCM
[rt,rn,Rx,Rn] = tSCM_offline(x,MNMF_it,Af,rt,rn,Rn_hat,alpha,beta,rou);        
% Multichannel Wiener filtering  仅限2路！
Y = zeros(I,J,M,ns);
for i = 1:I
    Rx_inv = inv(Rx(:,:,i));
    for j = 1:J
        Y(i,j,:,1) = rt(i,j) * AA(:,:,i) * Rx_inv * x(:,i,j);
