...
for f = 1:F
    ...
    [valu,num]=min(vall);b(:,f)= vec(:num+1);%b取最小非0特征值对应的特征向量，lamda取0特征值
    lamda_FT(f) = vall(num);
%     if vall(1) < 1e-20
...
for f = 1:F
    for t = 1:T
        Rx_inv = inv(Rx(:,:,f));
        rt_nume(f,t) = c_FT(f,t) * abs(X(:,f,t)' * Rx_inv * Af(:,nt,f)).^2 + beta / (rt(f,t).^2 + eps);% 分子(66) of [1]
        rt_deno(f,t) = Af(:,1,f)' * Rx_inv * Af(:,nt,f) + (alpha+1) / (rt(f,t) + eps);% 分母(66) of [1]
    end
end
end
...
for f = 1:F
    for t = 1:T
        Rx_inv = inv(Rx(:,:,f));
        rn_nume(f,t) = c_FT(f,t) * X(:,f,t)' * Rx_inv * Rn(:,:,f) * Rx_inv * X(:,f,t);% 分子(67) of [1]
