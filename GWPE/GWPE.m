...
function [dereverb_spec, window] = GWPE(wav, gwpe_config, fft_config)
...
[X_FTM, window] = STFT(wav, frame_len, frame_shift, 'hamming');
ffts = permute(X_FTM, [3, 2, 1]);
...
dereverb_spec = permute(x, [3, 2, 1]); % size: F x T x M
end

function irfft = irfft(x,fft_len,dim)
[T,L] = size(x); % Frame * Freqbin
n = 2 * (L - 1);
s = L - 1;
xn = zeros(T,n);
xn(:,1:L) = x;
xn(:,L+1:n) = conj(x(:,s:-1:2));
irfft = ifft(xn,fft_len,dim);
end