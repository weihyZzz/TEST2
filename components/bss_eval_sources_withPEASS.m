function [SDR,SIR,SAR,perm,option]=bss_eval_sources_withPEASS(se,s,option)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
s_true_all=[];e_spat_all=[];e_interf_all=[];e_artif_all=[];
...
for jest=1:nsrc,
    for jtrue=1:nsrc,
        [s_true,e_spat,e_interf,e_artif]=bss_decomp_mtifilt(se(jest,:),s,jtrue,1024);
        [SDR(jest,jtrue),SIR(jest,jtrue),SAR(jest,jtrue)]=bss_source_crit(s_true,e_spat,e_interf,e_artif);
    s_true_all = [s_true_all;s_true];e_spat_all = [e_spat_all;e_spat];e_interf_all = [e_interf_all;e_interf];e_artif_all = [e_artif_all;e_artif];
    end
end
...
s_true_out=s_true_all((0:nsrc-1).'*nsrc+perm,:);% nsrc * nsamples ( best ordering
e_spat_out=e_spat_all((0:nsrc-1).'*nsrc+perm,:);
e_interf_out=e_interf_all((0:nsrc-1).'*nsrc+perm,:);
e_artif_out=e_artif_all((0:nsrc-1).'*nsrc+perm,:);
option.s_true_out=s_true_out;option.e_spat_out=e_spat_out;option.e_interf_out=e_interf_out;option.e_artif_out=e_artif_out;
return;
